...
async def list_templates() -> list[dict]:
    out: list[dict] = []
    async for t in templates_coll().find().sort("_id", -1):
...
async def create_template(body: TemplateBody) -> dict:
    res = await templates_coll().insert_one({"text": body.text, "image": body.image})
...
async def delete_template(template_id: str) -> dict:
    ...
    res = await templates_coll().delete_one({"_id": ObjectId(template_id)})
